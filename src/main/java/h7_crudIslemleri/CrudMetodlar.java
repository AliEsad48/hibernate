...
public class CrudMetodlar {
    ...
    public void sessionFactoryOlustur() {
        ...
        try {
            ...
            Configuration con = new Configuration().
                    configure("hibernate.cfg.xml").addAnnotatedClass(Personel.class);
            sf = con.buildSessionFactory();

        } catch (Throwable e) {
            System.err.println("session fabrikası oluşurken hata oluştu" + e);

            throw new ExceptionInInitializerError(); //şart değil ama exception ın tam ismini yazmak istersek tercih edilebilir
    ...
    //veritabanındaki tüm kayıtları listeleyen metod
    ...
    //Veritabanından bir personeli silen
    ...
    //id ile bir kaydın maas bilgisini güncelle
    public void maasGuncelle(long id, int maas) {
        ...
        try {
            ...
            if (personel == null) {
                System.out.println("Girilen idye sahip bir personel bulunamadı");
            } else {
                personel.setMaas(maas);
                tx.commit();
                System.out.println(id + " id li personelin ueni maaşı " + maas);
            }
